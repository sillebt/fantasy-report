...
# Source configuration for dynamic paths
source("R/config.R")

# Helper function to get standings paths dynamically
get_standings_path <- function(year) {
  paste0("output/history/", year, "_standings.png")
}

# Get season range for titles
season_range <- get_season_range_label()
...
### Final Standings
<p style="text-align: center;">**All-Time Dynasty Standings & `r current_season` Final Regular Season Standings**</p>
```{r, echo=FALSE,out.width="49%",out.height="49%",fig.show='hold',fig.align='center', fig.cap="Standings"}
knitr::include_graphics(c("output/history/all_time_standings.png", get_standings_path(current_season)))
```
\newpage
<p style="text-align: center;">**`r current_season - 1` & `r current_season - 2` Final Regular Season Standings**</p>
```{r, echo=FALSE,out.width="50%",out.height="50%",fig.show='hold',fig.align='center', fig.cap="Standings"}
knitr::include_graphics(c(get_standings_path(current_season - 1), get_standings_path(current_season - 2)))
```
\newpage
### Notable Matchups
...
Hopefully, `r current_season + 1` doesn't require a tragic, near-death on live TV to determine its outcome. In a perfect world, it won't end with a tainted, asterisk-bound title. I think that's something we can all agree on - unlike saying, "Congrats, Tommy Tsao!"

With that - on to `r current_season + 1` and previewing each franchise's `r current_season + 1` roster, strengths, weaknesses, historical results and rivals. A Dynasty version of Why Your Team Sucks but, unlike Deadspin's review of the NFL, most of your guy's rosters are genuine, Oakland-level dogshit. 

_To preview each team, the following information is aggregated:_

1. `r current_season + 1` Pre-Draft Roster (Including Various Rankings)

2. `r current_season` Final Roster (Ranked by Fantasy Points Scored in Line-Up)

3. `r current_season` Final Schedule Win/Loss
